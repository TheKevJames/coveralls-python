...
from coveralls.configuration import ci
from coveralls.configuration import helpers
...
def test_defaults() -> None:
    ...
    assert config.host == helpers.DEFAULT_HOST
...
def test_to_payload_includes_only_set_payload_fields() -> None:
    config = configuration.Config(
        repo_token='xxx', service_name='travis-ci', parallel=True
    )
...
def test_to_payload_omits_unset_payload_fields() -> None:
    payload = configuration.Config(repo_token='xxx').to_payload()
...
def test_payload_and_client_fields_are_disjoint_and_cover_the_dataclass() -> (
    ...
    field_names = {f.name for f in dataclasses.fields(configuration.Config)}
    assert set(helpers.PAYLOAD_FIELDS).isdisjoint(CLIENT_ONLY_FIELDS)
    assert set(helpers.PAYLOAD_FIELDS) | set(CLIENT_ONLY_FIELDS) == field_names
...
def test_request_timeout_defaults() -> None:
    assert configuration.Config().request_timeout == (
        helpers.DEFAULT_CONNECT_TIMEOUT,
        helpers.DEFAULT_READ_TIMEOUT,
    )


def test_request_timeout_overall_applies_to_both_phases() -> None:
    assert configuration.Config(timeout=30).request_timeout == (30.0, 30.0)


def test_request_timeout_phase_specific_wins_over_overall() -> None:
    config = configuration.Config(
        timeout=30, connect_timeout=5, read_timeout=45
    )
...
def test_request_timeout_phase_specific_falls_back_to_default() -> None:
    ...
    assert configuration.Config(connect_timeout=5).request_timeout == (
        5.0,
        helpers.DEFAULT_READ_TIMEOUT,
    )
...
def test_retries_defaults_to_zero() -> None:
    assert configuration.Config().retries == 0
...
def test_retries_accepts_non_negative_integers(
    ...
    assert configuration.Config(retries=raw).retries == expected


@pytest.mark.parametrize('raw', ['abc', '1.5', '2.0', 2.5, 2.0, True, False])
def test_retries_rejects_non_integers(raw: Any) -> None:  # noqa: ANN401
    with pytest.raises(ValueError, match='must be an integer'):
        configuration.Config(retries=raw)


def test_retries_rejects_negative() -> None:
    with pytest.raises(ValueError, match='must not be negative'):
        configuration.Config(retries=-1)
...
def test_parse_pr_number(value: str | None, expected: str | None) -> None:
    ...
    assert ci._parse_pr_number(value) == expected
